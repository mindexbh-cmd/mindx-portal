...
### ADR-035: G20a parent-portal tab content restored via re-linking, not rewriting
- **Date**: 2026-05-22
- **Status**: accepted
- **Context**: On 2026-05-16, commit `3ad90c1` retired the ~100K-char flat-scroll `PARENT_HTML` template in favour of ~30K of split sub-page templates (one per parent-portal tab: hub / books / evaluations / fees / attendance / behaviour / etc.). The consolidation was correct in shape — sub-page templates are smaller, easier to reason about, and align with the G12 in-page-tab mechanism. But it silently dropped three user-visible features that lived in the OLD `PARENT_HTML`'s IIFE-scoped JS renderers and were not ported to the new sub-page templates:
  1. **المناهج (books) tab** — the read-only book viewer link. Books with `can_download=false` used to route to `/parent/book/<id>/viewer?pid=<pid>` (operator-built per-page WebP renderer with watermark, NO PDF download possible). After `3ad90c1`, every book — including IP-protected ones — pointed at `/api/books/<id>/view`, exposing the PDF binary and the browser's built-in download button.
  2. **النقاط (points) tab** — front-of-shop pending/rejected redemption callouts. The OLD `_ppRenderStore` IIFE rendered prominent blue ⏳ pending and orange ⚠ rejected cards (with the admin's `rejection_reason`) above the category tabs. The new sub-page shop pane had no equivalent — students/parents lost visibility into outstanding orders and rejection reasons.
  3. **التقييمات (evaluations) tab** — the richer pre-`3ad90c1` `PARENT_EVALUATIONS_HTML` (16K chars) had been replaced by a slimmer `PORTAL_PARENT_EVALUATIONS_HTML` (~13K chars) that dropped several presentation features.

  Critically, the **backend endpoints behind all three features were still alive the entire time**: `/parent/book/<bid>/viewer`, `/parent/book/<bid>/page/<n>.webp`, `/parent/evaluations/view`, and `/parent/evaluations` JSON were never touched by `3ad90c1` — only the frontend linkage to them was removed. The discovery loop for G20a (read `app.py` for the route handler before assuming a rewrite was needed) found this in minutes. The fix shape was therefore re-link, not rebuild.
- **Decision**: When migrating a flat-scroll template to sub-page templates, **prefer re-linking the OLD richer surface (via iframe wrapper, redirect, or direct URL) over re-porting features into the new template**, provided the OLD endpoints are still alive in the backend. Specifically:
  1. **The 3-tab content for the parent portal (books / evals / points) authoritatively lives at**: the OLD viewer routes (`/parent/book/<bid>/viewer` and `/parent/evaluations/view`) AND the in-tab template (`PORTAL_STUDENT_HTML`). Books and evaluations are now thin wrappers pointing at the rich pages; points-tab callouts were re-ported as ~50 lines of HTML/CSS into `PORTAL_STUDENT_HTML` (re-porting was cheap here because the data was already in `STATE.redemptions`).
  2. **Evaluations: iframe wrapper, not content port.** `/portal/parent-hub/evaluations?inner=1` returns a 492-char iframe wrapping `/parent/evaluations/view?pid=<pid>`. Standalone visit 302-redirects. Re-porting the full 16K-char `PARENT_EVALUATIONS_HTML` was rejected — ~600 lines of UI work for zero functional gain when an iframe achieves the same result.
  3. **Books: conditional URL switching, not content port.** The renderer now branches on `can_download`: `false` → `/parent/book/<id>/viewer?pid=<pid>`, `true` → `/api/books/<id>/view`. One line of template logic, restores IP protection completely.
  4. **Points: cheap re-port, because the data was already client-side.** `STATE.redemptions` was already populated; the callout cards are a static template render over that array. ~50 lines of HTML + CSS in `PORTAL_STUDENT_HTML`, matching the OLD `_ppRenderStore` pattern. Re-port was cheaper than introducing a new linking mechanism.
  5. **Backend untouched.** Zero route additions, removals, or behaviour changes. All 4 supporting endpoints (`/parent/book/<bid>/viewer`, `/parent/book/<bid>/page/<n>.webp`, `/parent/evaluations/view`, `/parent/evaluations` JSON) were alive since their creation date and remain unchanged.
  6. **The slimmed `PORTAL_PARENT_EVALUATIONS_HTML` template is now orphaned** (no callers) — flagged for a future cleanup pass. Do NOT delete in the same commit cycle as the wrapper switch; orphaned templates are cheap to carry and the deletion is an independent decision.
  7. **Future redesigns must check whether the OLD endpoint is still alive BEFORE removing its template.** Frontend templates are cheaper to rewrite than backend renderers; the surviving routes are the recovery hook. Document the backend dependency in the same commit that retires the frontend.
- **Alternatives considered**:
  - **Re-port the full OLD `PARENT_EVALUATIONS_HTML` into a new sub-page template**: rejected. ~600 lines of UI work; the iframe wrapper achieves the same user-visible result in ~5 lines of route handler. The iframe approach also keeps the OLD page authoritative for the data model — future evaluation-page changes touch one template, not two.
  - **Add a download-flag check to the existing `/api/books/<id>/view` endpoint** (have it 403 when `can_download=false`): rejected. The OLD per-page WebP viewer is a fundamentally different rendering pipeline (each page is watermarked, served as a separate image, blocks the browser's "save PDF" affordance entirely). A 403 on `/api/books/<id>/view` would just break IP-protected books for everyone with no replacement.
  - **Re-port the OLD `_ppRenderStore` JS verbatim into `PORTAL_STUDENT_HTML`** (rather than a fresh template render over `STATE.redemptions`): rejected. The OLD IIFE depended on `PARENT_HTML`-scoped globals that no longer exist; porting it as-is would require recreating a chunk of the dead template. The fresh template render reads `STATE.redemptions` directly and is ~50 lines.
  - **Three separate deploy cycles** (one tab per deploy): rejected by operator. The fixes are independent but the diagnostic frame is shared, the smoke suite covers all three together, and a single safe_deploy is cheaper than three. Locked in: "all 3 tabs sequentially in one deploy cycle".
  - **Soft-deprecate the slimmed `PORTAL_PARENT_EVALUATIONS_HTML` immediately** (delete in the same commit): rejected. Orphaned templates carry near-zero cost; the deletion decision is independent of the wrapper switch and belongs in a future cleanup pass alongside other dead-code removal.
  - **Roll back commit `3ad90c1` instead** (restore flat-scroll `PARENT_HTML`): rejected. The sub-page consolidation is the correct shape going forward; the regressions are localised to 3 specific renderer functions that were not ported. Reverting would lose all the G12 in-page-tab infrastructure built on top of the new sub-page templates.
- **Consequences**:
  - **The "re-link before you rebuild" diagnostic pattern is now codified.** When a parent-portal feature is reported lost, the first check is: does the OLD endpoint still exist in the backend? `git show 3ad90c1^1:app.py | grep -A 50 'function _ppRenderStore'` (or `_ppRender`, `ppRenderBookCard`, etc.) extracts the OLD render functions to confirm what the new template missed.
  - **Books tab IP protection is restored.** Books with `can_download=false` cannot be PDF-downloaded by parents. The watermarked per-page WebP viewer is back online.
  - **Points tab visibility is restored.** Students and parents see pending and rejected orders at the top of the shop pane, with the admin's `rejection_reason` inline for rejected cards. The "where did my order go?" support-load class drops.
  - **Evaluations tab depth is restored.** The richer 16K-char page (with the full evaluation history table, the score columns, the per-evaluation drill-down) is what parents see inside the tab — not the slimmed 13K-char version.
  - **`PORTAL_PARENT_EVALUATIONS_HTML` is dead code** (no callers) but retained. Future cleanup pass to remove it.
  - **The iframe wrapper pattern is now precedented.** Future tab content that needs to reuse a rich standalone page can follow the same shape: route returns a small iframe when called with `?inner=1`; standalone visit 302-redirects to the underlying page. Documented for reuse.
  - **Future redesigns must document backend dependencies.** When a template is retired, the same commit that retires it must list the backend routes it called and verify those routes are either also retired (intentional) or kept alive (intentional, with a re-linking plan).
  - **The diagnostic breadcrumb is logged.** The `3ad90c1` commit (2026-05-16) is now flagged in `CHANGE_LOG` G20a entry as the boundary where flat-scroll `PARENT_HTML` was retired — anything reported as "lost" in the 3 content tabs (books / evals / points) post-`3ad90c1` should first be diffed against `3ad90c1^1`.
- **Reference**: commits `f80e1f8` (G20a.1 books viewer link), `1df9f89` (G20a.2 points pending/rejected callouts), `f08138d` (G20a.3 evaluations iframe wrapper), `355b9d4` (G20a.4 hermetic test); CHANGE_LOG 2026-05-22 G20a entry; safety tags `safety/pre-restore-pre-redesign-features-2026-05-22` at `759a8d0` (pre-G20a baseline, real rollback point) and `safety/pre-g20a-parent-tab-restoration-20260522-022345` at `355b9d4` (safe_deploy tag); the boundary commit being restored from is `3ad90c1` (2026-05-16, "parent-portal V2 retirement / flat-scroll → sub-page templates consolidation").

### ADR-036: G20d balance display separates "still-owned" headline from "still-requestable" gate
- **Date**: 2026-05-22
- **Status**: accepted (supersedes the part of ADR-034 that pinned the single-number headline to `STATE.balance.available`; the rest of ADR-034 — single-number layout, no 3-card breakdown, internal accounting stays internal — stays in force; admin delivery-row UX section is independent of the balance-display section but bundled in the same ADR because both shipped in the same G20d wave per the operator's locked-in decisions)
- **Context**: ADR-034 (G19, 2026-05-22) collapsed the 3-card balance display (Total / قيد الحجز / المتاح) into a single big headline number, with the displayed value pinned to `STATE.balance.available` (= `total − committed − reserved`). Within hours of G19 landing, the operator surfaced a confusion case: a student with 217 points earned and 160 pts queued as a `requested` cart submission (awaiting admin approval) was shown a headline of `57`. The student's mental model was "I haven't spent anything yet — admin hasn't approved my order — why does my balance say 57?" The `reserved` field (G16 cart-aware: includes `requested` rows whose admin hasn't acted on yet) was technically correct accounting, but it conflated "still owned" with "still requestable". `requested` is a reservation, not a debit — the student still owns the points until admin approves and `committed` grows. Separately, the operator surfaced a second G20d UX issue: the `/points/manage` "في انتظار التسليم" tab's delivery flow used a single button that, on click, removed the row from the list via full reload. Admins lost visible confirmation their click landed; the row disappearing felt like an error state. Both issues were independent in cause but bundled in the same deploy cycle because both were UX visibility failures in the same admin/parent points pipeline.
- **Decision**: **Pick a balance field per purpose. The display answers "what do I still own?"; the gate answers "what can I still queue?". Different fields, same backend dict.**
  1. **Student rewards headline shows `STATE.balance.total − STATE.balance.committed`.** Excludes `reserved`. A pending cart request (`status='requested'`) does NOT reduce the displayed number. Only when admin approves (flips `requested → pending`, growing `committed`) does the headline drop. The headline animates this delta via the `countUp(_headlineNum, prev, next)` helper.
  2. **Cart over-request gate stays internal on `STATE.balance.available`** (= `total − committed − reserved`). The G15.6 insufficient-balance modal, the G16.4 proactive cart-aware check, the G20b.1 unaffordable-card shading, and the server-side `/cart/add` + `/cart/checkout` validations all consume `.available`. The smaller `.available` figure is hidden from the student; the larger `total − committed` is what they see.
  3. **Backend untouched.** `_g15_student_balance` still returns `{total, committed, reserved, available}` with the same arithmetic identity (`available = total − committed − reserved`). The renderer consumes different fields. No schema migration. No route signature change. The Python helper's contract is preserved so future tooling can opt into the breakdown.
  4. **Admin delivery row stays visible after click, with checkbox + button side-by-side.** The `/points/manage` "في انتظار التسليم" list renders each row with a `.pd-cb` checkbox + `.pd-btn` button — both wired to `pdDeliver(rid)`. After a successful deliver: checkbox set to `checked`, button text → "✅ مُسلَّم", button `disabled`, button background `#9e9e9e`, `.pd-row` `opacity: 0.75`. The row is NOT removed from the DOM. Only the pending-delivery badge count refreshes. Next page load filters out `status='delivered'` rows server-side so the row drops naturally on navigation.
  5. **Future balance-display work follows the two-tier pattern.** When designing a new display or gate that reads from `/api/portal/student/balance`, document which of the 4 fields it consumes and why. Mixing fields (e.g. showing `.available` as the headline + gating cart on the same field) is what caused the operator confusion that motivated G20d.
- **Alternatives considered**:
  - **Show `STATE.balance.total` as the headline (gross earnings, ever)**: rejected. Equivalent to the pre-G19 3-card top card. Doesn't drop when admin approves a redemption — a student who actually spent 80 pts on a delivered prize would still see their gross number, which is operationally misleading. The headline must drop when the points are truly gone.
  - **Show `STATE.balance.available` as the headline (G19's original choice)**: rejected by operator. Caused the G20d-motivating confusion: pending `requested` cart submissions reduce `.available` immediately via the `reserved` field, but students don't experience that as "spent" — they experience it as "queued, admin hasn't decided yet". The display must lag the gate by the `reserved` amount.
  - **Add tooltips / inline explanations** explaining "your balance is 57 because 160 is reserved in your pending cart request": rejected. Tooltips on a 360px mobile viewport are unreliable; inline explanation text is fragile and exposes internal accounting that should be invisible. ADR-034's "internal accounting stays internal" principle still applies.
  - **Adjust the backend `_g15_student_balance` helper to fold `reserved` into `committed`** so `available = total − committed` straight: rejected. Breaks the cart-aware gate which still needs to distinguish "admin-approved committed" from "student-queued reserved" (the latter can be cancelled trivially via cart deletion; the former requires admin action). Keeping the 4-key dict and picking fields per purpose is cleaner.
  - **Hide the delivery row from the admin list immediately on click** (the pre-G20d behaviour): rejected. Admins reported "I clicked deliver but the row disappeared — did the click register?" The visible checked + gray success state is operator-tested confirmation. The row drops on navigation anyway, so the "stays visible until refresh" window is short.
  - **Fade-out animation on delivered row**: rejected by operator. Animation invites questions about whether the row "is still being delivered" vs "delivered". Static gray success state is unambiguous.
  - **Three separate deploy cycles** (one per fix): rejected. The two fixes share a diagnostic frame (UX visibility failures in the same pipeline), the smoke suite covers both, and a single `safe_deploy` is cheaper than two. Locked in: G20d.1 + G20d.4 in one deploy cycle.
- **Consequences**:
  - **Students see the headline they expect.** A student with earned points + a pending cart request sees their gross-minus-truly-debited number, matching their mental model of "what I still own". The headline drops on admin approval, not on cart submission.
  - **The cart over-request gate continues to enforce truth-in-spending.** A student with 217 earned + 160 reserved cannot queue another 100-pt request — the server-side validator on `/cart/add` and `/cart/checkout` rejects with the G15.6 insufficient-balance modal. The gate is invisible (hidden behind `.available`) but still tight.
  - **Admins get visible delivery confirmation.** The row's gray checked state is unambiguous — the click landed, the API succeeded, the row will drop on next load. No more "did it work?" follow-ups.
  - **The two-tier "display vs gate" pattern is now codified.** Future features that read the balance payload must pick a field per purpose and document why. Mixing them in the same surface is a known-bad pattern.
  - **ADR-034's single-number layout stays in force** — only the field being displayed changed. The 3-card breakdown is still rejected; the "internal accounting stays internal" principle is still locked in.
  - **No backend test, no schema migration, no helper change.** All G20d work is frontend-only. Future shop or balance-display features can build on the same 4-key payload.
  - **Test infrastructure tightened**: new `scripts/smoke_g20d.py` (40 checks) inverse-asserts the old headline consumer (`bavail`, `b.available`) is gone, positive-asserts the new consumer (`_headlineNum`, `b.total - b.committed`), AND positive-asserts the cart gates still use `.available` internally. `scripts/smoke_g19.py` has two assertions updated to accept the new headline consumer. All 10 smoke suites pass (G12 + G13 + G14 + G15 + G16 + G17 + G19 + G20a + G20b + G20d).
  - **Supersedes the part of ADR-034 that pinned the headline to `.available`**; everything else from ADR-034 (single-number layout, no 3-card breakdown, internal accounting stays internal, cart gates use `.available`, backend untouched) stays in force.
- **Reference**: commits `1153b09` (G20d.1 headline formula), `1b51793` (G20d.4 delivery checkbox + row-stays-visible), `b0d56c2` (G20d.5 hermetic test), `f9163628` (G19 smoke assertion update); CHANGE_LOG 2026-05-22 G20d entry; safety tags `safety/pre-g20d-balance-display-delivery-2026-05-22` at `b531c9b` (real pre-G20d rollback point) and `safety/pre-g20d-headline-delivery-20260522-033710` at `f9163628` (safe_deploy bookmark); supersedes the headline-field portion of ADR-034.
