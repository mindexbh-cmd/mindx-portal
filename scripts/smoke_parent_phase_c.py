...
# ── [17] v2.9.10.4-safe: cart-checkout writes status='requested'
#        so points DON'T debit on order placement (matches the
#        legacy direct-order path). The pending-list query stays
#        in its expanded form (matching both 'requested' AND
#        'pending') for backward-compat with any rows still in
#        'pending' from before this fix. The cancel endpoint
#        already accepts both statuses.
assert ("WHERE student_id=? AND status IN ('requested','pending')"
        in SRC), (
    "pending-list query reverted from the v2.9.10.3 expanded "
    "form — cart orders or any historical 'pending' rows would "
    "vanish from قيد المراجعة")
ck_idx = SRC.index("def api_parent_cart_checkout_v2")
ck_body = SRC[ck_idx:ck_idx + 4000]
assert '"requested", "parent_cart"' in ck_body, (
    "cart checkout no longer writes status='requested' — points "
    "would debit on placement, breaking the v2.9.10.4-safe "
    "no-debit-until-approval invariant")
assert '"pending", "parent_cart"' not in ck_body, (
    "cart checkout still writes 'pending' somewhere — that path "
    "regresses to the immediate-debit behavior")
...
print("[17] Cart checkout writes status=requested (no immediate "
      "debit); pending-list still includes both statuses; cancel "
